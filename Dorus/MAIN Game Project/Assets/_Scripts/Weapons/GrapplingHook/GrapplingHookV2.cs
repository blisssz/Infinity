...
public class GrapplingHookV2 : MonoBehaviour {
	...
	private bool ropeShootPaused = false;
	private bool ropeRetractPaused = false;
	...
	void Update(){
		if (PauseMenu.paused == true) {
			gameWasPaused = true;
			if(ropeShoot.isPlaying == true){
				ropeShoot.Pause();
				ropeShootPaused = true;
			}
			if(ropeRetract.isPlaying == true){
				ropeRetract.Pause();
				ropeRetractPaused = true;
			}
		}
		else{
			if(gameWasPaused == true){
				if(ropeShootPaused == true){
					Debug.Log ("was paused");
					ropeShoot.Play();
					ropeShootPaused = false;
				}
				if(ropeRetractPaused == true){
					Debug.Log ("was paused");
					ropeRetract.Play();
					ropeRetractPaused = false;
				}
				gameWasPaused = false;
			}
		}
