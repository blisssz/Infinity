...
//	private int shootState = Animator.StringToHash("Base Layer.SNIPERSHOT");
	...
	private const int startingMagazines = 10;
	...
	void Start () {
		...
		playerMouselook = this.transform.root.GetComponent<MouseLook> ();
	}
	...
	void LateUpdate () {
		if(gunEquipped == true){
			...
			if (fire == true && timeStamp <= Time.time && aimhold == true && reloading == false) {
			...
			if (aim == true) {	//Left shift is pressed (runs only once when held down)
				anim.SetBool(zoomHash, true);
//				Camera.main.GetComponent<IndieEffects>().enabled=true;
			}
			else if(aimhold == true){	//Left shift is held down, aim
				if(reloading == true){
					...
					if(playerMouselook != null){
						float sensitivity = MapValues(sniperCamera.fieldOfView, 1f, 9.45f, 0.5f, 2f);
						playerMouselook.sensitivityX = sensitivity; //2.5 is standard
						playerMouselook.sensitivityY = sensitivity;
					}else{
						playerMouselook = this.transform.root.GetComponent<MouseLook> ();
					}
					anim.SetBool(zoomHash, true);
			...
			else if(aimRelease == true){	//Left shift is released
				anim.SetBool(zoomHash, false);
				playerMouselook.sensitivityX = 2.5f;
				playerMouselook.sensitivityY = 2.5f;  
