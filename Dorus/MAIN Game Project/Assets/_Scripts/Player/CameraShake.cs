...
public class CameraShake : MonoBehaviour {
	...
	void Start () {
		...
		Delta=GameObject.FindWithTag("Player").GetComponent<Movement1>().DeltaPosition;
	}
	...
	void Update () {
		...
		this.transform.position = this.transform.parent.position + coInWorld+Delta;
		
		for (int i = 0; i < otherCams.Length; i++){
			otherCams[i].transform.position = this.transform.parent.position+Delta + 0.5f*coInWorld;
		}
