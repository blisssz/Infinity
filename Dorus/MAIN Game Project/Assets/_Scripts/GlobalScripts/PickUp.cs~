						...
						if (PickUpNumber == 0) {
								...
								if (Weapon.GetComponent<Gun> ()) {
										switch(PlayerManager.useWeaponID){
											...
											case 5:
												//SMG
												Weapon.GetComponent<Gun> ().addMagazines(1);
												Weapon.GetComponent<Gun> ().addAmmunition(50);
												break;
											case 6:
												//handgun
												Weapon.GetComponent<Gun> ().addMagazines(5);
												break;
											default:
												Debug.Log ("Something went wrong, no gun attached to player");
												break;
										}
