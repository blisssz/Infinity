...
public class MouseLook : MonoBehaviour {
	...
	public float gunRotationY = 0f;
	public float gunRotationX = 0f;
	...
	{	
		...
		// yaw
		transform.Rotate( new Vector3(0.0f, (Input.GetAxis("Mouse X") * sensitivityX) + gunRotationX, 0.0f) );
		gunRotationX = 0f;

		// pitch, note that rot_y is positive when looking down
		float rot_y = (-Input.GetAxis("Mouse Y") * sensitivityY) + gunRotationY;
		gunRotationY = 0f;
