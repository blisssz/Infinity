		...
		void Move (int o)
		{
				...
				for (int i=0; i<Moves[o].Length; i++) {
						...
						if (!lastPositions [0].Equals (Vector3.zero)) {
								...
								if (up == 0) {
										Instantiate (CheckPoint, lastPositions [0] + CheckPointOffset, Quaternion.identity);
										up = 30;
								}
								if(iteration == 0){
									Instantiate(endPoint, lastPositions[0] + CheckPointOffset, Quaternion.identity);
								}
								up--;
								iteration--;
						}
			float Chance=0.5f;
				if(spawnEnemy){
				enemySpawn.SpawnObject(Position, Chance, "Coin");	
				}
			Chance=0.1f;
				if(true){enemySpawn.SpawnObject (Position, Chance, "Enemy");
			}
			}
