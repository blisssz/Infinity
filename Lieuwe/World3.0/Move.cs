...
public class Move {
	...
	public void Choose ()
	{
		...
		case 0:
			...
			float Distance = (float)rand.NextDouble ()*(maxDistance-minDistance)+minDistance;
			AngleChanger = Quaternion.Euler (0, HelpScript.Rand (-100, 100) + Direction.eulerAngles.y - 90, HelpScript.Rand (-30, 30));
		...
		case 1:
			Update = (Moves.Sphere (StartPosition, 12, 0));
			moved = false;
			clear = false;
			Cannot.Add(1);
			break;
		...
		case 2:
			minDistance = 10;
			maxDistance = 25;
			Distance =  HelpScript.Rand(minDistance, maxDistance);
